...
/**
 * An <code>Individual</code> consists of a genotype and tags. Tags can store
 * arbitrary data and are used i.e. by parameter controllers to mark certain
 * individuals. Moreover, an individual knows its latest fitness value and its
 * date of birth.<br>
 * 
 * What is important here is that individuals are to be handled read-only.
 * Modules that intend to alter attributes such as the genotype or tags have to
 * clone the individual and work on the clone instead. Altering the original
 * individual directly can cause unpredictable results in other parts of FrEAK.
 *
...
public class Individual implements Cloneable, Serializable {
	 ...
	 */
	public Individual(ScheduleInterface schedule, Genotype genotype, Individual[] parents) {
	...
	/**
	 * Sets the latest known fitness value. If the value <code>null</code> is
	 * specified, the fitness value is assumed to be unknown.
	 * This method is be used only by the fitness function to provide a cache
	 * if the fitness value for a particular individual does not vary.
	 *
	...
	/**
	 * Returns the latest known fitness value or <code>null</code> if unknown.
	 * This is only a cache that may only be used by the fitness function
	 * if the fitness value for a particular individual does not vary.
	 * To get the correct fitness value, <code>FitnessFunction.evaluate</code>
	 * must be called.
	 *
