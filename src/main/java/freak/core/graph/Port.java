...
/**
 * <code>Port</code>s are interfaces between <code>Operator</code>s.
 * Also two Ports imply an edge in the <code>OperatorGraph</code> if they are
 * connected (which means that the partner attributes point to each other).
 * Ports always belong to one Operator.
 * 
 * @author Matthias
 */
