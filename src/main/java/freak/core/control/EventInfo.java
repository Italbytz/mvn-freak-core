...
/**
 * Contains information about an event-mapping. This class knows the textual
 * description of an event, the type and a source of it.
 * <p>
 * It also distinguishes between static and non-static events:
 * <p>
 * A so called <i>static</i> event is an event for which the source cannot be
 * changed during runtime, e.g. it won't be displayed in the GUI as event with
 * variable source.
 * <p>
 * A <i>non-static</i> event is an event for which the eventsource cannot be
 * known during runtime. The user has to configure the eventsource before the
 * event ise useable.
 * <p>
 * An <code>EventInfo</code> object usually doesn't come alone. It is part of
 * the EventController which stores a list of events for every module in the
 * program.
 * 
 * @author Oliver
 */
public class EventInfo implements Serializable {
	...
	/**
	 * Sets the event source for the event. This throws a <code>RunTimeException</code>
	 * if the event is a static event.
