...
public class Replay implements Serializable {
	 ...
	 */
	ScheduleInterface getCheckPointFor(GenerationIndex timeIndex) {
		try {
			SortedMap head = upTo(checkPoints, timeIndex);
			Object key = head.lastKey();
			CheckPoint point = (CheckPoint)head.get(key);
			ScheduleInterface copy = (ScheduleInterface)StreamCopy.copy(point.schedule);
			return copy;
	 ...
	 */
	void addPoint(ScheduleInterface activeSchedule, boolean isEditPoint) {
	...
	private void createCheckPoint(GenerationIndex timeIndex, ScheduleInterface schedule) {
		try {
			byte[] streamedSchedule = StreamCopy.serialize(schedule);
			ScheduleInterface copy = (ScheduleInterface)StreamCopy.read(streamedSchedule);
	...
	private static class CheckPoint implements Serializable {
		...
		public CheckPoint(ScheduleInterface schedule, int memoryRequirement) {
